...
class CeattyresspiderSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        driver.find_element_by_xpath(
            '//*[@id="get-callback"]/div/div/div[1]/button'
        ).click()
        print("CLOSING POP UPS AND DIALOGUE BOXES")
        time.sleep(3)

        driver.find_element_by_xpath(
            "/html/body/div[5]/div/div[1]/header/div/div/div[1]/div[2]/ul/li[1]"
        ).click()
        ...
        for post in postals:
            ...
            listt = driver.find_element_by_xpath('//*[@id="parentNode"]').get_attribute(
                "outerHTML"
            )
            ...
            for data in templatenode:
                ...
                dealer_code = data.find("span", {"class": "icon icon-whatsapp"}).get(
                    "data-dealercode"
                )
                ...
                if dealer_code not in keys:
                    ...
                    ####################
                    try:
                        manufacturer_unique_id = data.find(
                    ...
                    try:
                        if (
                            data.find("span", {"class": "ceat-shoppe-label"}).get(
                                "style"
                            )
                            == "display: none;"
                        ):
