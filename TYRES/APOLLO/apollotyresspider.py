...
class ApollotyresspiderSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.apollotyres.com/en-in/car-suv-van/dealer-finder/find-a-dealer/"
    ]
    ...
    def parse(self, response, **kwargs):
        ...
        for post in postals:
            ...
            driver.find_element_by_xpath("//input[@itemprop='codeCountry']").send_keys(
                post
            )
            ...
            page_source = driver.find_element_by_xpath(
                "//div[@id='dealer-result']"
            ).get_attribute("outerHTML")
            print("SOURCE CODE OF CLASS CONTAINING DEALERS LIST FETCHED")

            soup = BeautifulSoup(page_source, "lxml")
            search_container = soup.find_all(
                "div", {"itemtype": "https://schema.org/location"}
            )
            ...
            for div in search_container:
                if div["data-id"] != "current-position-id":
                    ...
                    if dealer_code not in keys:
                        ...
                        try:
                            full_address = (
                                div.find("div", {"itemprop": "addressRegion"})
                                .find("span")
                                .text
                            )
                        except Exception as e:
                        ...
                        try:
                            if (
                                div.find(
                                    "div",
                                    {
                                        "ng-class": "{'has__apolloZone':dealer.IsApolloZone()}"
                                    },
                                ).get("class")
                                is None
                            ):
