...
class AngelmatchspiderSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for page in range(6):
            next_page = "https://angelmatch.io/?q=&hPP=3&idx=demo_INVESTORS&p=" + str(
                page + 0
            )
            ...
            for data in ais_hits_item:
                ...
                try:
                    name = data.find(
                        "h1", {"class": "hit-title card-header-title"}
                    ).text
                ...
                try:
                    company_name = data.find(
                        "h1",
                        {
                            "style": "flex-grow:0;color: purple;font-size: 1.1rem;font-weight:bold; padding: 0;"
                        },
                    ).text
                except Exception as e:
                    company_name = ""
                ...
                try:
                    links = {}
                    linkslist = data.find("div", {"class": "social-icons"}).find_all(
                        "a", {"target": "_blank"}
                    )
                    for link in range(len(linkslist)):
                        links.update(
                            {"link_url_" + str(link + 1): linkslist[link].get("href")}
                        )
                except Exception as e:
                    links = ""
                ...
                try:
                    investment_focuses = {}
                    investment_focuses_list = data.find_all(
                        "span",
                        {
                    ...
                    for inv_foc in range(len(investment_focuses_list)):
                        investment_focuses.update(
                            {
                                "investment_focuses_"
                                + str(inv_foc + 1): investment_focuses_list[
                                    inv_foc
                                ].text
                            }
                        )
                ...
                try:
                    ...
                    companies_invested_in_list = data.find_all(
                        "span", {"style": "margin-top:5px; "}
                    )
                    for com_int_in in range(len(companies_invested_in_list)):
                        companies_invested_in.update(
                            {
                                "companies_invested_in_"
                                + str(com_int_in + 1): companies_invested_in_list[
                                    com_int_in
                                ].text
                            }
                        )
                ...
                try:
                    ...
                    companies_invested_in_links_list = data.find_all(
                        "span", {"style": "margin-top:5px; "}
                    )
                    for com_int_in_link in range(len(companies_invested_in_links_list)):
                        companies_invested_in_links.update(
                            {
                                "companies_invested_in_links_"
                                + str(
                                    com_int_in_link + 1
                                ): companies_invested_in_links_list[com_int_in_link]
                                .find("a")
                                .get("href")
                            }
                        )
