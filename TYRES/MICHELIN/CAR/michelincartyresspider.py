from michelincartyres.MichelincartyresItem import MichelincartyresItem
import time
import scrapy
...
class MichelincartyresspiderSpider(scrapy.Spider):
    name = "michelincartyresspider"
    start_urls = ["https://www.michelin.in/auto/dealer-locator/400059+IN"]

    def parse(self, response, **kwargs):
        items = MichelincartyresItem()
        keys = list()
        ...
        for post in postals:
            ...
            driver.find_element_by_xpath('//*[@id="b2c-dl-search__input"]').send_keys(
                post
            )
            ...
            while True:
                ...
                time.sleep(5)
                if "display: none;" in driver.find_element_by_xpath(
                    '//*[@id="b2c-dl-result-load-more"]'
                ).get_attribute("style"):
                    ...
                    driver.find_element_by_xpath(
                        '//*[@id="b2c-dl-result-load-more"]/div[2]/span[2]'
                    ).click()

            print("GETTING SOURCE CODE OF CLASS CONTAINING DEALERS LIST")
            result_list = driver.find_element_by_xpath(
                '//*[@id="b2c-dl-result-list"]'
            ...
            for li in blocks:
                ...
                if dealer_code not in keys:
                    ...
                    print(
                        "DEALER CODE NOT PRESENT IN THE LIST SO APPENDING --> ",
                        dealer_code,
                    )
                    ...
                    try:
                        store_name = li.find(
                            "div", {"class": "b2c-dl-result-card-title__dealer-name"}
                        ).text
                    except Exception as e:
                        store_name = ""
                    ...
                    ####################
                    try:
                        full_address = li.find(
                            "div", {"class": "b2c-dl-result-card-infos__dealer-address"}
                        ).text
                    except Exception as e:
                        full_address = ""
                    ...
                    try:
                        if (
                            li.find(
                                "div",
                                {"class": "b2c-dl-result-card-infos__dealer-phone"},
                            )
                            is None
                        ):
                            phone_number = ""
                        else:
                            phone_number = li.find(
                                "div",
                                {"class": "b2c-dl-result-card-infos__dealer-phone"},
                            ).text
                    except Exception as e:
                    ...
                    try:
                        ...
                        print(
                            "Exception while getting michelin_certified_centre --> ", e
                        )
                        pass
                    ...
                    ####################
                    try:
                        google_maps_direction_url = li.find(
                            "a", {"data-analytics": "DL_CLICK_DEALER_DIRECTION"}
                        ).get("href")
                    except Exception as e:
                        google_maps_direction_url = ""
                        print(
                            "Exception while getting google_maps_direction_url  --> ", e
                        )
                        pass
                    ...
                    try:
                        if (
                            li.find("a", {"data-analytics": "DL_CLICK_DEALER_WEBSITE"})
                            is not None
                        ):
                            website = li.find(
                                "a", {"data-analytics": "DL_CLICK_DEALER_WEBSITE"}
                            ).get("href")
                        else:
                            website = ""
                    except Exception as e:
                    ...
                    try:
                        if (
                            ...
                            appointment_booking_url = li.find(
                                "a",
                                {
                                    "data-analytics": "DL_CLICK_DEALER_WEBSITE_APPOINTMENT"
                                },
                            ).get("href")
                        else:
                        ...
                        appointment_booking_url = ""
                        print(
                            "Exception while getting appointment_booking_url  --> ", e
                        )
                        pass
                    ...
                    ####################
                    try:
                        brand_website_store_url = "https://www.michelin.in" + li.find(
                            "div", {"class": "b2c-dl-result-card-infos__dealer-details"}
                        ).find("a").get("href")
                    except Exception as e:
                        brand_website_store_url = ""
                        print(
                            "Exception while getting brand_website_store_url  --> ", e
                        )
                        pass
