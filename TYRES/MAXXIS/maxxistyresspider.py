...
class MaxxistyresspiderSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        dealers_list = driver.find_element_by_xpath(
            '//*[@id="dealers-list"]'
        ...
        for div in search_container:
            ...
            ####################
            try:
                full_address = div.p.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling.text.replace(
                    "Address:", ""
                )
            except Exception as e:
                full_address = ""
            ...
            try:
                phone_number = (
                    div.p.next_sibling.next_sibling.next_sibling.text.replace(
                        "Mobile:", ""
                    )
                )
            except Exception as e:
            ...
            ####################
            try:
                pincode = div.p.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling.text.replace(
                    "Pincode -", ""
                )
            except Exception as e:
                pincode = ""
