    ...
    def parse(self, response, **kwargs):
        ...
        try:
            title = driver.find_element_by_xpath(
                '//*[@id="product-layout-helper"]/div[3]/strong'
            ).text
        except Exception as e:
            title = ""
        ...
        try:
            sku = driver.find_element_by_xpath(
                '//*[@id="product-layout-helper"]/div[5]/div[1]/div[2]/div[1]/strong'
            ).text
        except Exception as e:
            sku = ""
        ...
        try:
            brand = driver.find_element_by_xpath(
                '//*[@id="product-layout-helper"]/div[5]/div[1]/div[2]/div[2]/strong'
            ).text
        except Exception as e:
            brand = ""
        ...
        try:
            if (
                driver.find_element_by_xpath(
                    '//*[@class="prod-desc-content section-content"]'
                ).text
                == None
            ):
                desc = ""
        ...
        try:
            images = {}
            product_imgurls = driver.find_elements_by_xpath(
                "/html/body/main/div[2]/div[2]/div[1]/div[3]/div/img"
            )
            for img_urls in range(len(product_imgurls)):
                images.update(
                    {
                        "image_url_"
                        + str(img_urls + 1): product_imgurls[img_urls].get_attribute(
                            "src"
                        )
                    }
                )
        ...
        try:
            docs = {}
            document_urls = driver.find_elements_by_xpath(
                '//*[@id="prod-manuals-list"]/div/p/a'
            )
            for doc_urls in range(len(document_urls)):
                docs.update(
                    {
                        "doc_url_"
                        + str(doc_urls + 1): document_urls[doc_urls].get_attribute(
                            "href"
                        )
                    }
                )
        ...
        try:
            specs = {}
            key = driver.find_elements_by_xpath(
                '//*[@id="prod-spec-list"]/div[1]/div/div[@class="feature-type nowrap"]/strong'
            )
            value = driver.find_elements_by_xpath(
                '//*[@id="prod-spec-list"]/div[1]/div/div[@class="feature-value"]'
            )
