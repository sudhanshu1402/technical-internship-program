...
class GoodyeartyresspiderSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        while True:
            ...
            listt = driver.find_element_by_xpath('//*[@id="store-list"]').get_attribute(
                "outerHTML"
            )
    ...
    def parse_items(self, response):
        ...
        try:
            store_name = response.xpath(
                '//*[@id="wapper"]/div/div/div/div/div[1]/div[1]/div/div[1]/p/strong/text()'
            ).extract_first()
        except Exception as e:
            store_name = ""
        ...
        try:
            full_address = response.xpath(
                '//*[@id="wapper"]/div/div/div/div/div[1]/div[1]/div/div[1]/p/span/text()'
            ).extract_first()
        except Exception as e:
            full_address = ""
        ...
        try:
            if (
                response.xpath(
                    '//*[@id="wapper"]/div/div/div/div/div[1]/div[1]/div/div[2]/div[1]/p[2]/a/text()'
                ).extract_first()
                == None
            ):
                email_id = ""
            else:
                email_id = response.xpath(
                    '//*[@id="wapper"]/div/div/div/div/div[1]/div[1]/div/div[2]/div[1]/p[2]/a/text()'
                ).extract_first()
        except Exception as e:
            email_id = ""
        ...
        try:
            phone_number = response.xpath(
                '//*[@id="wapper"]/div/div/div/div/div[1]/div[1]/div/div[2]/div[1]/p[1]/span/text()'
            ).extract_first()
        except Exception as e:
            phone_number = ""
        ...
        try:
            if (
                response.xpath(
                    '//*[@id="wapper"]/div/div/div/div/div[1]/div/div/div[3]/text()'
                ).extract_first()
                == "Goodyear Retailer"
            ):
                goodyear_zone = "1"
