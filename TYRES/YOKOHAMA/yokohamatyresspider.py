        ...
        for option_state in options:
            ...
            for option_city in options:
                ...
                for box in boxes:
                    ...
                    store_results = driver.find_element_by_xpath(
                        '//*[@id="map"]/div/div/div[2]/div[3]/div/div[4]'
                    ).get_attribute("outerHTML")
                    store_results_soup = BeautifulSoup(store_results, features="lxml")

                    try:
                        check = (
                            store_results_soup.find("div", {"id": "bodyContent"})
                            .find("p")
                            .text
                        )
                    ...
                    if check not in keys:
                        ...
                        try:
                            ...
                            manufacturer_unique_id = ""
                            print(
                                "Exception while getting manufacturer_unique_id --> ", e
                            )
                            pass
                        ...
                        try:
                            if (
                                "Email"
                                in store_results_soup.find("div", {"id": "bodyContent"})
                                .find("p")
                                .find_next("p")
                                .find_next("p")
                                .find_next("p")
                        ...
                        try:
                            if (
                                store_results_soup.find("span", {"class": "ycn_bg"})
                                is not None
                            ):
                                yokohama_zone = "1"
                            else:
                        ...
                        try:
                            google_maps_direction_url = (
                                store_results_soup.find("div", {"id": "bodyContent"})
                                .find("p")
                                .next_sibling.get("href")
                            )
                        except Exception as e:
                            google_maps_direction_url = ""
                            print(
                                "Exception while getting google_maps_direction_url --> ",
                                e,
                            )
                            pass
                        print(
                            "google_maps_direction_url -> ", google_maps_direction_url
                        )
                        items["google_maps_direction_url"] = google_maps_direction_url
                        ####################
                        ####################
                        try:
                            if "Mr" in store_results_soup.find(
                                "div", {"id": "bodyContent"}
                            ).find_next("p").find_next("p").find_next("p"):
                                dealer_name = (
                                    store_results_soup.find(
                                        "div", {"id": "bodyContent"}
                                    )
